...
-- [ IsQuestUnit ]
-- Returns a pfQuest tooltip icon for quest related units on the current map.
function IsQuestUnit(name)
  if not name or not IsAddOnLoaded("pfQuest") or not pfMap or not pfMap.tooltips or not pfQuestConfig then
    return
  end
  ...
  if pfMap.tooltips[name] then
    ...
    if meta["quest"] then
      ...
      return pfQuestConfig.path .. "\\img\\cluster_mob"
    end
  end
...
local function RunQueue()
  ...
  if not next(queue) and frameRunning then
    frameRunning = false
...
function ShaguPlatesX.api.RunOOC(func)
  ...
  if not queue[tostring(func)] then
    ...
    if not frameRunning then
      frameRunning = true
      frame:SetScript("OnUpdate", RunQueue)
    end
    return true
  end
end
...
function ShaguPlatesX.api.GetItemLinkByName(name)
  ...
  -- Safe mode: Disabled to prevent 1.12 client severe freeze.
  -- Scanning 25000 uncached items blocking the engine for several seconds.
  return nil
end
...
-- [ Create Border ]
-- Creates a custom border texture for nameplates
function ShaguPlatesX.api.CreateBorder(f, size)
  if not f.backdrop then
    local border_size = size or 8
    
    -- 边框框架作为血条父框架的子框架，而不是血条的子框架
    local parent = f:GetParent()
    b = CreateFrame("Frame", nil, parent or f)
    b:SetPoint("TOPLEFT", f, "TOPLEFT", -border_size, border_size)
    b:SetPoint("BOTTOMRIGHT", f, "BOTTOMRIGHT", border_size, -border_size)
    -- 边框框架和血条使用相同的绝对层级
    local parentLevel = f:GetFrameLevel()
    b:SetFrameLevel(parentLevel)

    b.bdf = CreateFrame("Frame", nil, b)
    b.borbd = b.bdf:CreateTexture(nil, "BACKGROUND")
    b.borbd:SetAllPoints(f)
    b.bdf:SetFrameLevel(parentLevel > 1 and (parentLevel - 1) or 0)

    -- 边框纹理使用 ARTWORK 层，和血条 StatusBar 在同一层级
    b.bor = b:CreateTexture(nil, "ARTWORK")
    b.bor:SetTexture("Interface\\AddOns\\ShaguPlatesX\\img_f\\Border")
    b.bor:SetAllPoints(b)
    b.bor:SetTexCoord(0, 0.8515625, 0.52125, 1)
    b.bor:SetVertexColor(1, 1, 1, 1)

    f.backdrop = b
  end
end

function ShaguPlatesX.api.SetBorderColor(f, r, g, b, a)
  if f.backdrop and f.backdrop.bor then
    f.backdrop.bor:SetVertexColor(r, g, b, a)
  end
end
