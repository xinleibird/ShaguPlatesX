ShaguPlatesX:RegisterModule("cooldown", "vanilla", function ()
  ...
  local function pfCooldownOnUpdate()
    ...
    parent_name = this.parent_name or parent:GetName()
    ...
    if this.start < now then
      ...
      local unixTime = time()
      local startupTime = unixTime - now
      ...
      local remaining = cdEndTime - unixTime
  ...
  local function SetCooldown(this, start, duration, enable)
    ...
    and start > 0 and duration > 0 and (not enable or enable > 0) then
      ...
      this.pfCooldownText.parent_name = this:GetParent() and this:GetParent():GetName()
